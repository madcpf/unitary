...
class Move:
    """Container class that has the source and target of a quantum chinese chess move.

    If the move is a split move, it will have a target2.  If a merge move,
    it will have a source2 attribute.

    For moves that are input from the quantum chinese chess board API, this will
    ...
    def from_string(cls, str_to_parse: str):
        ...
        if "^" in source_target:
            ...
            if len(source_target) != 7 or len(sources_str) not in [2, 4]:
